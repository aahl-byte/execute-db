...
def read_env_text(env: str, env_path: Path) -> str:
    ...
    Plaintext environments are allowed in every mode, including hardened (system)
    mode. Under hardening the file itself is still unreadable to your account (it
    lives under the service user); a plaintext env only means *using* it does not
    prompt for a password — the query runs straight through the launcher. Encrypt
    an env with `password set` if you want a per-use password gate.
    """
