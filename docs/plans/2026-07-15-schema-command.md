...
**The message must be GENERIC — do not name the write.** `except OSError` at `main` is broad enough to also catch a store `OSError` (e.g. `read_bytes()` on an unreadable `.env.dev`), which `tests/test_schema.py::test_a_store_failure_is_not_relabelled_as_an_introspection_failure` deliberately lets escape. Wording the handler write-specifically — *"could not write the schema to stdout: …"* — would report an unreadable env file as a write failure: exactly the mislabelling that boundary test exists to prevent, relocated one layer up. Exit 1 with a generic line (`execute-db: [Errno 28] No space left on device`) is the right outcome for both sources.
